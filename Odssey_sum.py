...
data = json.load(open('/data_sdc/data/SimpAgent-main/eval_results/GUIOdyssey/Baseline4AO_nohistory/Baseline4AO_lora_225_drop_28.json', 'r'))

# 兼容多种 JSON 最外层结构：
# 1) {"outputs": [...]} / {"results": [...]} / {"data": [...]} 
# 2) 直接是 [...]
...
for record in gt_test:
    ...
    for key in pred_dict:
        ...
        if key == "click_point":
            ...
            if calculate_f1_score(pred_value, gt_value)>0.5:
                matching = True
...
for record in gt_test:
    try:
        ...
        if pred_action_type is not None:
            pred_action_type_key = str(pred_action_type)
            pred_type_counts[pred_action_type_key] = pred_type_counts.get(pred_action_type_key, 0) + 1
    except:
        continue

print(f"{'Predicted Type':<15} {'Count':<8} {'Percentage':<10}")
print("-" * 50)
for action_type in sorted(pred_type_counts.keys(), key=lambda x: (0, int(x)) if str(x).isdigit() else (1, str(x))):
