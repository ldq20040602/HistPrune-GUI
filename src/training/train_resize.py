...
import ast
from transformers import AutoConfig, AutoProcessor, AutoModelForVision2Seq, BitsAndBytesConfig, HfArgumentParser
from src.training.trainer import QwenTrainer
from src.training.data_resize import make_supervised_data_module
...
def print_params_with_name(model):
    total_params = sum([p.numel() for p in model.parameters()])
    trainable_params = sum([p.numel() for p in model.parameters() if p.requires_grad])
