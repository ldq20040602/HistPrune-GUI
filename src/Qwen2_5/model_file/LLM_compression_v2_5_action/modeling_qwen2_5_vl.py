...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: Qwen2_5_VLConfig):
        ...
        self._n_before_prefill: Optional[int] = None
    ...
    def _random_build_keep_mask(self, OAS_index, input_ids, pad_token_id: int = 151643):
        ...
        device = input_ids.device
        keep_mask = torch.ones(B, L, dtype=torch.bool, device=device)
        keep_mask &= (input_ids != pad_token_id)
    ...
    def _DivPrune_selection(self, indices_abs, hidden_states_b, target_n, device):
        N = indices_abs.numel()
        ...
        # Max-Min 距离策略：选择与当前已选集合“最小距离”最大的 token
        # 距离 d=1-cos，等价于最小化到已选集合的最大余弦相似度
    ...
    def _DivPrune_build_keep_mask(self, hidden_states, OAS_index, DivPrune_info, input_ids):
        B, L, H = hidden_states.shape
        device = hidden_states.device
        pad_token_id = 151643
        keep_mask = torch.ones(B, L, dtype=torch.bool, device=device)

        # 不再依赖 Sobel 先验：在每张历史截图段内部直接进行整体贪心选择
        ...
        for b in range(B):
            ...
            for img_id, (s, e) in enumerate(hist_segments):
                ...
                if self.DivPrune_ratios and len(self.DivPrune_ratios) > 0:
                    dist = (T - 1) - img_id
                    budget = self.DivPrune_ratios[dist] if dist < len(self.DivPrune_ratios) else self.DivPrune_ratios[-1]
                else:
                    budget = 1.0

                budget = 0.0 if budget < 0.0 else (1.0 if budget > 1.0 else budget)
                keep_n = int(budget * n_valid)
                if keep_n <= 0:
                    keep_n = 1
    ...
    def forward(
        ...
        for i, decoder_layer in enumerate(self.layers):
            if i == 0:
                # 仅在 prefill (seq_len > 1) 时刷新 FLOPs 统计，避免 decode (seq_len == 1)
                # 覆盖掉刚完成的 prefill 统计结果。
            ...
            if (self.enable_DivPrune_pruning or self.enable_fastv_pruning or self.enable_pdrop_pruning or 
            ...
            # DART: 基于特征多样性，直接使用 hidden_states
            elif self.enable_dart_pruning and (i == (self.drop_k - 1)) and hidden_states.shape[1] != 1:
                try:
            ...
            elif (self.enable_fastv_pruning or self.enable_pdrop_pruning or self.enable_uigraph_pruning or self.sobel_enable or 
                ...
                try:
                    ...
                    if self.sobel_enable and (self._sobel_info is not None):
                        ...
                        self._pdrop_keep_mask = self._pdrop_build_keep_mask(
