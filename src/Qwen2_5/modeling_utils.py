...
# Optional integrations (allow minimal local fork without integrations/ directory)
try:
    ...
    class PeftAdapterMixin:  # minimal fallback
        pass
    ...
    def _load_state_dict_into_zero3_model(*args, **kwargs):
        raise RuntimeError("DeepSpeed integration is unavailable in this minimal build.")

    def flash_attention_forward(*args, **kwargs):
        raise RuntimeError("flash_attention integration is unavailable in this minimal build.")

    def flex_attention_forward(*args, **kwargs):
        raise RuntimeError("flex_attention integration is unavailable in this minimal build.")

    def sdpa_attention_forward(*args, **kwargs):
        raise RuntimeError("sdpa_attention integration is unavailable in this minimal build.")

    # Keep minimal compatibility with configs carrying tensor parallel style hints.
    # We only use this for validation bypass in non-TP local inference.
    SUPPORTED_TP_STYLES = {"colwise", "rowwise"}

    def shard_and_distribute_module(*args, **kwargs):
        raise RuntimeError("tensor_parallel integration is unavailable in this minimal build.")

# Optional loss mapping
try:
...
# Optional quantization stack
try:
    from .quantizers import AutoHfQuantizer, HfQuantizer
    from .quantizers.quantizers_utils import get_module_from_name
except Exception:
    AutoHfQuantizer = None

    class HfQuantizer:  # minimal fallback
        pass

    def get_module_from_name(module, tensor_name: str):
        # Minimal fallback used by weight loading paths.
        if "." in tensor_name:
